...
function formatLiteralValue(value, python = true) {
    if (value === null)
        return python ? 'None' : 'null';
    if (value === undefined)
        return python ? 'None' : 'undefined';
    ...
    if (isTypeNode(value))
        return typeNodeToString(value, 3);
    try {
...
function formatSample(sample) {
    ...
    if (sample === null)
        return 'null';
    const config = vscode.workspace.getConfiguration('trickle');
    const maxLen = config.get('sampleLength', 200);
